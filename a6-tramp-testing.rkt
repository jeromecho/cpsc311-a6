...
(define (lookup-env env0 x0)
  (type-case Env env0
    [emptyE ()
            (error/311 'interp/kra/kdtd "Unbound Identifier: ~a" x0)]
    [extend-env (env x v)
                (if (symbol=? x x0)
                    v
                    (lookup-env env x0))]
...
(define TH0 (in-for-r-app/th (numV 1) (numV 3) (init/k)))
(define TH1 (in-for-nought-app/th (numV 0)
                                  (for-pred/k (id 'x)
                                              (id 'y)
                                              (extend-env empty-env 'x (numV 1))
                                              (init/k))))
(define TH2 (in-app-interp/th (id 'f)
                              (id 'y)
                              (extend-env-fixFun
                               (extend-env empty-env 'y (numV 2))
                               'f
                               'x
                               (id 'x))
                              (for-l/k (num -4)
                                       (extend-env-fixFun
                                        ...
                                        (id 'x))
                                       (init/k))))
; runtime error - using numV as rator 
(define TH3 (in-funv-apply/th (ifB (nought? (id 'x))
                                   (add (id 'x) (num -1))
                                   (add (id 'x) (num 7)))
                              'x
                              (numV 0)
                              empty-env
                              (for-rator/k (num 1) empty-env (init/k))))
; no runtime error, using valid funV as rator
(define TH4 (in-funv-apply/th (ifB (nought? (id 'x))
                                   (fixFun 'f 'x (id 'x))
                                   (add (id 'x) (num 7)))
                              'x
                              (numV 0)
                              empty-env
                              (for-rator/k (num 1) empty-env (init/k))))
; function body has free identifier has
(define TH5 (in-funv-apply/th (ifB (nought? (id 'x))
                                   (fixFun 'f 'x (id 'y))
                                   (add (id 'x) (num 7)))
                              'x
                              (numV 0)
                              empty-env
                              (for-rator/k (num 1) empty-env (init/k))))
; thunk execution doesn't terminate 
(define TH6 (in-funv-apply/th (ifB (nought? (id 'x))
                                   (fixFun 'f 'x (app (id 'f) (id 'x)))
                                   (add (id 'x) (num 7)))
                              'x
                              (numV 0)
                              empty-env
...
(tramp-define
 (apply/kra/kdtd v1 v2 k)
 (type-case Value v1
   [funV (x body env)
         (bounce (in-funv-apply/th body x v2 env k))]
   [else (error/311 'apply/kra/kdtd "Bad function: ~a" v1)]))
...
(define (interp/kra/kdtd kra)
  (mount-trampoline
   (interp/kra-env/kdtd kra empty-env (init/k)))) ; stub for the examples
